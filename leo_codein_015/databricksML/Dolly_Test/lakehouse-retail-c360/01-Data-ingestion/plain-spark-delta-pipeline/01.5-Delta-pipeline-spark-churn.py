...
# MAGIC To run this demo, just select the cluster `dbdemos-lakehouse-retail-c360-leo_huang` from the dropdown menu ([open cluster configuration](https://adb-5679407200568336.16.azuredatabricks.net/#setting/clusters/0523-174019-egalu70/configuration)). <br />
...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Flakehouse%2Flakehouse-retail-c360%2F01-Data-ingestion%2Fplain-spark-delta-pipeline%2F01.5-Delta-pipeline-spark-churn&cid=5679407200568336&uid=3857247757561488">
...
(spark.readStream 
     ...
     .writeStream
        .option("checkpointLocation", f"{cloud_storage_path}/checkpoint/churn_users")
...
(spark.readStream 
        ...
        .drop(col("_rescued_data"))
     .writeStream
        .option("checkpointLocation", f"{cloud_storage_path}/checkpoint/churn_orders")
        .option("mergeSchema", "true")
        .trigger(availableNow = True)
...
# MAGIC
# MAGIC
# MAGIC Switch to Databricks SQL to see how this data can easily be requested using <a href='/sql/dashboards/6d0277b7-9ab5-444c-b04f-017308aedae9' target="_blank">Churn prediction DBSQL dashboard</a>, or an external BI tool. 
# MAGIC
