...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE main.dbdemos_retail_c360.ml_churn_features AS SELECT * FROM main.dbdemos_retail_c360.churn_features;
...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("main.dbdemos_retail_c360.dlt_expectations")

# COMMAND ----------

spark.sql(f'alter table main.dbdemos_retail_c360.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
