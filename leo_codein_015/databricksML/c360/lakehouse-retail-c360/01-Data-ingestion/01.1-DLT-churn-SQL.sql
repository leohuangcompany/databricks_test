...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=4472318973269700&notebook=%2F01-Data-ingestion%2F01.1-DLT-churn-SQL&demo_name=lakehouse-retail-c360&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-retail-c360%2F01-Data-ingestion%2F01.1-DLT-churn-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/29920bcb-d95e-4139-809d-42ddad189543" target="_blank">Churn Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/main/dbdemos_retail_c360/c360/users'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming orders (json)
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/main/dbdemos_retail_c360/c360/orders'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming clickstream (csv)
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/main/dbdemos_retail_c360/c360/events', header=True))
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360/c360/events", "csv", map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360/c360/orders", "json")
...
AS SELECT * FROM cloud_files("/Volumes/main/dbdemos_retail_c360/c360/users", "json", map("cloudFiles.inferColumnTypes", "true"))
...
-- MAGIC #                                                                 Model name         |        
-- MAGIC #                                                                     |              |        
-- MAGIC predict_churn_udf = mlflow.pyfunc.spark_udf(spark, "models:/main.dbdemos_retail_c360.dbdemos_customer_churn@prod", "int")
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-churn" href="#joblist/pipelines/29920bcb-d95e-4139-809d-42ddad189543" target="_blank">Churn Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/9c031c55-f7ab-41c1-8935-e15599cbf1e2-dlt---retail-data-quality-stats" target="_blank">Data Quality Dashboard</a>
...
-- MAGIC
-- MAGIC <a href="/sql/dashboards/8d3fd289-6d19-4559-a44e-6a18a41e3d3b" target="_blank">Open the DBSQL Dashboard</a>
